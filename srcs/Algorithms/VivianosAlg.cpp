...
class VivianosAlg : public Algorithm {
    ...
    std::vector<std::tuple<int, Path>> computePath(Field field, std::vector<DroneData> drones) override {
        ...
        std::pair<int, int> temp = wrk_field.getCCPosition();
        Coordinates cc_pos = Coordinates(temp.first, temp.second);

        int autonomy = 15000; // TODO: controllare correttezza
        ...
        for (auto drone : drones) {
            while (true) {
                ...
                autonomy -= ManhattanDistance(cc_pos, cur_pos);
    ...
    void goToCurPos(Coordinates bho) { 
        Coordinates bohh = bho;
        std::cout << "Vai a " << bohh.x << " " << bohh.y << std::endl;
    } // TODO: 
