 ...
 */
string createGroup(redisContext *context, const string &stream, const string &group, bool mkstream = true) {
...
string sendJsonMessage(redisContext *context, const string &stream, const json &jsonObject) {
    ...
    auto *reply = (redisReply *) redisCommand(context, "XADD %s * json %s", stream.c_str(), jsonObject.dump().c_str());
    if (reply == nullptr) {
 ...
 * @return
 */
Response readMessageGroup(redisContext *context, const string &group, const string &consumer, const string &stream, int block = -1) {
    // Read a message from a group
    // Create the XREADGROUP command
    string blockStr = block >= 0 ? "BLOCK " + to_string(block) : "";
    string xreadgroup = "XREADGROUP GROUP " + group + " " + consumer + " COUNT 1 "  + blockStr + " STREAMS " + stream + " >";
...
//        cerr << "readMessageGroup: Error: Expected an array or empty" << endl;
 ...
 long ackMessage(redisContext *context, const string &stream, const string &group, const string &messageId) {
     // Acknowledge a message
     auto *reply = (redisReply *) redisCommand(context, "XACK %s %s %s", stream.c_str(), group.c_str(), messageId.c_str());
     if (reply == nullptr) {
 ...
 * @return group info
 */
GroupInfo getInfoGroup(redisContext *context, const string &stream, const string &nameGroup) {
